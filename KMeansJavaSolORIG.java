...
class KMeansJava {
	...
	public static void main(String[] args) throws NumberFormatException {
		...
		List<List<Double>> centroids2 = sc.textFile(args[2]).map(line -> Arrays.asList(line.split(" ")).stream().map(Double::valueOf).collect(Collectors.toList())).collect();

		final int n = Integer.parseInt(args[3]);
		List<Double> cost1 = new ArrayList<>(), cost2 = new ArrayList<>();
		...
		System.out.println(cost1); System.out.println(cost2);
	}
